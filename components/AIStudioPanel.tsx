...
export const AIStudioPanel: React.FC<AIStudioPanelProps> = ({
  ...
  prompt, setPrompt, handleRefineImage, handleGenerate, updateStyle, onFileUpload
}) => {
    ...
    <div className="space-y-10 animate-in slide-in-from-right-4 duration-500 pb-20">
      {isImageSelected ? (
        <div className="space-y-12">
          {/* SECCIÓN 0: ORIGEN DE LA IMAGEN */}
          <section className="space-y-4">
            <div className="flex items-center gap-3 px-1">
              <Icon name="image" size={18} className="text-zinc-400" />
              <label className="text-[11px] text-zinc-400 font-black uppercase tracking-[0.2em]">Image Source</label>
            </div>
            <button 
              onClick={onFileUpload} 
              className="w-full py-5 bg-indigo-600 border border-indigo-400 rounded-3xl text-white font-black uppercase tracking-widest text-[10px] hover:bg-indigo-500 transition-all flex items-center justify-center gap-3 shadow-xl"
            >
              <Icon name="plus" size={18} /> Upload from PC
            </button>
          </section>

          {/* SECCIÓN 1: AJUSTES MANUALES */}
          <section className="space-y-6">
            <div className="flex items-center gap-3 px-1">
              <Icon name="filter" size={18} className="text-indigo-400" />
              <label className="text-[11px] text-indigo-400 font-black uppercase tracking-[0.2em]">Manual Adjustments</label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {presets.map((p) => (
                <button 
                  ...
                  className={`py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest border transition-all ${selectedElement?.style?.filter === p.filter ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-white/5 border-white/5 text-zinc-500 hover:text-white hover:bg-white/10'}`}
                >
                  {p.name}
                </button>
              ))}
          ...
          <section className="space-y-6 pt-6 border-t border-white/5">
            <div className="flex items-center gap-3 px-1">
              ...
              <label className="text-[11px] text-indigo-400 font-black uppercase tracking-[0.2em]">AI Lab</label>
            </div>
            <button 
              onClick={() => handleRefineImage("Make background transparent")} 
            ...
            >
              <Icon name="maximize" size={18} /> AI Background Erase
            </button>
          </section>
        </div>
      ) : (
        <section className="space-y-6">
          <div className="flex items-center gap-3 px-1">
            ...
            <label className="text-[12px] text-indigo-400 font-black uppercase tracking-[0.2em]">Component Architect</label>
          </div>
          <div className="relative">
            <textarea 
              placeholder={`Describe transformations for ${selectedElement?.name}...`} 
              className="w-full bg-black/40 border border-white/10 rounded-[32px] p-8 text-xs focus:border-indigo-600 outline-none h-48 resize-none shadow-inner transition-all font-medium"
              value={prompt}
            ...
            <button 
              ...
              className="absolute bottom-6 right-6 p-5 bg-indigo-600 rounded-2xl shadow-2xl hover:translate-y-[-2px] transition-all disabled:opacity-50"
            >
